...
public class GeoPoint {
	...
	// using ints for your internal representation of GeoPoint. 
  	...
  	/**
  	 * Return the difference of latitude in km.
     * @requires the latitude in millionths
  	...
  	public double headingTo(GeoPoint gp) {
		 ...
		 // increase in the counterclockwise direction. 
  		...
  		if ((diff_longitude > 0) && (diff_latitude < 0))
  			h = h + 90;
  		else if ((diff_longitude < 0) && (diff_latitude < 0))
  			h = h + 180;
  		else if ((diff_longitude < 0) && (diff_latitude > 0))
  			h = h + 270;
  		return h;
     ...
     **/
  	public boolean equals(Object gp) {
  		...
  		if (gp instanceof GeoPoint == false)
  			return false;
  		
  		GeoPoint gp1 = (GeoPoint) gp;
  		if (this.getLatitude() != gp1.getLatitude())
  			return false;
  		if (this.getLongitude() != gp1.getLongitude())
  			return false;
  		return true;
  	}
     ...
     * @return a hash code value for this GeoPoint.
  	...
  	public int hashCode() {
    	...
    	// for improved performance.
  		...
  		// Convert the latitude and the longitude to a single integer using linear function
    	return (m_latitude-GeoPoint.MIN_LATITUDE)*(GeoPoint.MAX_LONGITUDE-GeoPoint.MIN_LONGITUDE)+
    			m_longitude-GeoPoint.MIN_LONGITUDE;
  	}
